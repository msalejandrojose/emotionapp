...
describe("Bombergame", function () {
  ...
  it("Registrar el usuario Test", function (done) {
    juego.registrarUsuario({ email: "test", nick: "test", contrasena: "test" }, function (data) {
      //data.res=="ok"
      expect(data.nick).toBe("test");
      expect(data.email).toBe("test");
      expect(cifrado.encrypt("test")).toBe(data.contrasena);
      done();
    });
  });

  it("Iniciar Sesion el usuario Test", function (done) {
    juego.iniciarUsuario({ email: "test", contrasena: "test"}, function (data) {
      //data.res=="ok"
      expect(data.nick).toBe("test");
      expect(data.email).toBe("test");
      expect(cifrado.encrypt("test")).toBe(data.contrasena);
